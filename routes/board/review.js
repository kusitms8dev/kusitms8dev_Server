...
router.get("/", async (req, res, next) => {
  try {
    const selectReviewBoardQuery =
      "SELECT * FROM Board WHERE category = 'review'";
    const selectReviewBoardResult = await db.queryParam_None(
      selectReviewBoardQuery
    );

    if (!selectReviewBoardResult[0]) {
      return res
        .status(200)
        .send(
          defaultRes.successTrue(
            statusCode.OK,
            resMessage.EMPTY_REVIEW_BOARD_LIST,
            []
          )
        );
    }
    ...
    return res
      ...
      .send(
        defaultRes.successTrue(
          statusCode.OK,
          resMessage.SUCCESS_SELECT_REVIEW_BOARD_LIST,
          selectReviewBoardResult
        )
      );
  } catch (error) {
    console.log(error);
    return res
      ...
      .send(
        defaultRes.successFalse(
          ...
          resMessage.FAIL_SELECT_REVIEW_BOARD_LIST
        )
      );
  }
});
...
router.post("/", async (req, res) => {
  try {
    const insertReviewBoardQuery =
      "INSERT INTO Board (category, title, content, date, writer) VALUES (?, ?, ?, ?, ?)";
    const insertReviewBoardResult = await db.queryParam_Arr(
      insertReviewBoardQuery,
      [
        "review",
        req.body.title,
        req.body.content,
    ...
    return res
      ...
      .send(
        defaultRes.successTrue(
          statusCode.OK,
          resMessage.SUCCESS_REGISTER_REVIEW_BOARD
        )
      );
  } catch (error) {
    return res
      ...
      .send(
        defaultRes.successFalse(
          statusCode.INTERNAL_SERVER_ERROR,
          resMessage.FAIL_REGISTER_REVIEW_BOARD
        )
      );
  }
});
